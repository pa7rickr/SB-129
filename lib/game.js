...
// Tebak Matematika
const addmath = (chatId, jawaban, expired, points, _db) => {
    let obi = { id: chatId, jawaban: jawaban, expired: Date.now() + toMs(`${expired}s`), points: points }
    _db.push(obi)
}
...
const getPointsMath = (chatId, _db) => {
    let found = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === chatId) {
            found = i
